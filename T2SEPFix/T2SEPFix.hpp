...
// BAR indices (as passed to mapDeviceMemoryWithRegister / configRead)
...
static constexpr uint32_t kT2ReplyCountMask  = 0xF;   // 4 bits → 0-15 replies
...
static constexpr uint8_t kPCICapPCIeExp  = 0x10;

// MSI Message Control bits (offset 2 from MSI capability base)
...
static constexpr uint16_t kMSICtrlMMEMask     = 0x0070;  // multiple-message enable
static constexpr uint16_t kMSICtrlMME3Vectors = 0x0020;  // enable log2(8)=3 → 8 vectors
...
// waitForMatchingService timeout (ms)
static constexpr uint32_t kNVRAMWaitTimeoutMs = 8000;
...
// Initialises the device and probes its mailbox.
// ---------------------------------------------------------------------------
...
private:
    IOPCIDevice         *mT2Device;         // weak ref — owned by IOKit
    IOMemoryMap         *mBAR4Map;          // strong ref — retained by us

    // --- Helpers ---

    // Log PCI identity, BARs, and command register of the T2 device.
    void        logDeviceInfo(IOPCIDevice *dev);

    // Set PCI command register bits to ensure Memory Space + Bus Master are set.
    // Also calls changePowerStateTo(0) to ensure D0 (if power management attached).
    ...
    // Stores the mapping in mBAR4Map (caller must release on stop).
